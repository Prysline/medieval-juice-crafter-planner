...
describe('partial delivery atomic commit', () => {
  ...
  it('persists a same-recipe terminal serving after selling one initial serving and one refilled serving', () => {
    ...
    const storage = new MemoryStorage({
      [INVENTORY_STORAGE_KEY]: JSON.stringify(inventory),
      [STORAGE_KEYS.suppliedToday]: JSON.stringify([]),
    })
    const plan: DeliveryExecutionPlan = {
      planFingerprint: 'terminal-refill-plan',
      policy: 'retain-and-wash',
      trips: [
        {
          tripNumber: 1,
    ...
    expect(readInventoryState(storage).juiceJars[0]).toEqual({
      id: 'jar-a',
      recipeId: 'recipe-a',
      servings: 1,
    })
