import type { Customer, Preference } from '../types'

const ingredient = (value: string): Preference => ({ kind: 'ingredient', value })
const effect = (value: string): Preference => ({ kind: 'effect', value })

export const customers: Customer[] = [
  { id: 'galiana', name: '加莉安娜', occupation: '麵包師', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [ingredient('橙子'), effect('增強免疫'), effect('煥亮肌膚')] },
  { id: 'otilde', name: '奧蒂爾德', occupation: '女釀酒師', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [ingredient('橙子')] },
  { id: 'ulrich', name: '烏爾里希', occupation: '管家', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('增強免疫')] },
  { id: 'alia', name: '阿莉婭', occupation: '魚販', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('煥亮肌膚'), effect('增強免疫')] },
  { id: 'leticia', name: '萊蒂西亞', occupation: '孩童', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('酸味')] },
  { id: 'katrin', name: '卡特琳', occupation: '薄荷商人', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('酸味'), effect('增強免疫')] },
  { id: 'viviana', name: '維維安娜', occupation: '女僕', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [ingredient('檸檬')] },
  { id: 'konrad', name: '康拉德', occupation: '漁夫', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('酸味')] },
  { id: 'maximus', name: '馬克西姆斯', occupation: '錫匠', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('甜味'), effect('補充精力')] },
  { id: 'lizzy', name: '莉齊', occupation: '售糖商人', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [ingredient('橙子'), ingredient('薄荷')] },
  { id: 'pierre', name: '皮埃爾', occupation: '旅行者', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('清新口氣'), effect('增強免疫')] },
  { id: 'barti', name: '巴蒂', occupation: '孩童', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [ingredient('薄荷'), effect('甜味')] },
  { id: 'fulbertus', name: '富爾貝圖斯', occupation: '麵包師', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('清新口氣')] },
  { id: 'lizzie', name: '莉姬', occupation: '貴婦', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('甜味'), effect('增強免疫')] },
  { id: 'thomas', name: '托馬斯', occupation: '漁夫', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('甜味'), effect('舒緩腸胃')] },
  {
    id: 'jack',
    name: '傑克',
    occupation: '帽匠',
    villageId: 'east-harbor',
    satisfactionRequired: 0,
    preferences: [ingredient('橙子')],
    schedule: [{ type: 'leave_home', approxTime: '08:10～08:19', note: '離開家門後再走一小段才會實際離村。' }],
  },
  { id: 'harry', name: '哈里', occupation: '木匠', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('酸味'), effect('舒緩腸胃'), effect('清新口氣')] },
  {
    id: 'nanette',
    name: '娜內特',
    occupation: '魚販',
    villageId: 'east-harbor',
    satisfactionRequired: 0,
    preferences: [ingredient('檸檬'), effect('增強免疫')],
    schedule: [
      { type: 'leave_home', approxTime: '08:30', note: '離開家門後再走一小段才會實際離村。' },
      { type: 'return_village', approxTime: '23:10' },
    ],
  },
  { id: 'betsy', name: '貝茜', occupation: '旅店店主', villageId: 'east-harbor', satisfactionRequired: 150, preferences: [effect('酸味'), effect('清新口氣'), effect('補充精力')] },
  { id: 'zenobia', name: '澤諾比婭', occupation: '孩童', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [ingredient('紅蘿蔔'), ingredient('梨')] },
  { id: 'yolanda', name: '約蘭達', occupation: '編籃匠', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [ingredient('橙子'), ingredient('檸檬')] },
  { id: 'christian', name: '克雷斯蒂安', occupation: '貴婦', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [ingredient('梨'), effect('促進消化')] },
  { id: 'tricus', name: '特里庫斯', occupation: '孩童', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [ingredient('梨'), effect('補充精力')] },
  { id: 'ralph', name: '拉爾夫', occupation: '旅店侍者', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [ingredient('紅蘿蔔')] },
  { id: 'william', name: '威廉', occupation: '旅店侍者', villageId: 'east-harbor', satisfactionRequired: 0, preferences: [effect('促進消化'), effect('舒緩腸胃')] },
  {
    id: 'ivo',
    name: '伊沃',
    occupation: '蔬果商',
    villageId: 'east-harbor',
    satisfactionRequired: 120,
    preferences: [ingredient('香蕉'), effect('改善視力')],
    schedule: [
      { type: 'outside_village_by', approxTime: '06:00', note: 'PC 起床時已在村外，出家門時間未知。' },
      { type: 'return_village', approxTime: '22:00' },
    ],
  },
  { id: 'patricia', name: '帕特里夏', occupation: '貴婦', villageId: 'east-harbor', satisfactionRequired: 200, preferences: [ingredient('橙子'), effect('紓解壓力'), effect('酸味')] },
  { id: 'derrick', name: '德里克', occupation: '領主', villageId: 'east-harbor', satisfactionRequired: 250, preferences: [effect('清新口氣'), effect('改善視力')] },
  { id: 'eric', name: '埃里克', occupation: '碼頭總管', villageId: 'east-harbor', satisfactionRequired: 200, preferences: [effect('改善視力'), effect('調節血糖')] },
  { id: 'peter', name: '彼得', occupation: '蔬果商', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [effect('調節血糖'), effect('酸味')] },
  { id: 'wilmetta', name: '威爾梅塔', occupation: '悲劇商人', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [effect('保護心臟'), effect('增強免疫')] },
  { id: 'pauline', name: '波林', occupation: '孩童', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('香蕉'), ingredient('薄荷')] },
  { id: 'ricon', name: '裡孔', occupation: '鐵桶匠', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [effect('促進消化'), effect('保護心臟')] },
  { id: 'betty', name: '貝蒂', occupation: '地毯商人', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('肉桂'), effect('補充精力')] },
  { id: 'solomon', name: '所羅門', occupation: '放貸人', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('丁香'), effect('舒緩呼吸')] },
  { id: 'ingrid', name: '英格麗德', occupation: '檸檬商人', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('香蕉'), effect('增強免疫'), effect('補充精力')] },
  { id: 'emerys', name: '埃梅莉絲', occupation: '香料商人', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('紅蘿蔔'), effect('紓解壓力')] },
  { id: 'sarah', name: '薩拉', occupation: '孩童', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('橙子'), ingredient('香蕉')] },
  { id: 'nicholas', name: '尼可拉斯', occupation: '陶匠', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('橙子'), effect('芳香')] },
  { id: 'tacy', name: '塔茜', occupation: '畫師', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [effect('輔助瘦身'), effect('甜味')] },
  { id: 'valerian', name: '瓦勒里安', occupation: '孩童', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('梨'), ingredient('香蕉')] },
  { id: 'gavinus', name: '加維努斯', occupation: '馬夫', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('檸檬'), effect('輔助瘦身')] },
  { id: 'avina', name: '阿維娜', occupation: '女釀酒師', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [effect('輔助瘦身')] },
  { id: 'lila', name: '莉拉', occupation: '女釀酒師', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('紅蘿蔔'), effect('清新口氣'), effect('甜味')] },
  { id: 'hugo', name: '雨果', occupation: '酒館侍者', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('梨'), effect('改善視力'), effect('酸味')] },
  { id: 'ulbert', name: '烏爾伯特', occupation: '旅行者', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('肉桂'), effect('酸味'), effect('保護心臟')] },
  { id: 'macarius', name: '馬卡里烏斯', occupation: '旅行者', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('黃瓜'), effect('補充精力')] },
  { id: 'jacob', name: '雅各', occupation: '床具商人', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [effect('調節血糖'), effect('紓解壓力')] },
  { id: 'christina', name: '克里斯蒂娜', occupation: '麵包師', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('紅蘿蔔'), effect('芳香')] },
  { id: 'florida', name: '弗洛莉婭', occupation: '孩童', villageId: 'tranquil-fountain', satisfactionRequired: 0, preferences: [ingredient('橙子'), ingredient('肉桂')] },
  { id: 'daniel', name: '丹尼爾', occupation: '酒館店主', villageId: 'tranquil-fountain', satisfactionRequired: 200, preferences: [effect('奶香'), effect('保護心臟'), effect('促進消化')] },
  { id: 'octavius', name: '奧克塔維烏斯', occupation: '領主', villageId: 'tranquil-fountain', satisfactionRequired: 300, preferences: [ingredient('肉桂'), effect('改善視力'), effect('煥亮肌膚')] },

  { id: 'julia', name: '茱莉婭', occupation: '蔬果商', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('補充水分'), effect('輔助瘦身'), effect('增強免疫')] },
  { id: 'rolf', name: '羅爾夫', occupation: '伐木工', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('奶香'), effect('芳香')] },
  { id: 'gisela', name: '吉塞拉', occupation: '貴婦', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('強身健體'), effect('酸味'), effect('輔助瘦身')] },
  { id: 'martha', name: '瑪莎', occupation: '蔬果商', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [ingredient('番茄'), effect('調節血糖'), effect('芳香')] },
  { id: 'harvey', name: '哈維', occupation: '鞋匠', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [ingredient('丁香'), effect('甜味'), effect('保護心臟')] },
  { id: 'charles', name: '查爾斯', occupation: '香料商人', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('辛香'), effect('補充精力')] },
  { id: 'bernard', name: '伯納德', occupation: '農夫', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('調節血糖'), effect('芳香'), effect('辛香')] },
  { id: 'dominic', name: '多米尼克', occupation: '農夫', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('舒緩腸胃'), effect('甜味'), effect('保護心臟')] },
  { id: 'gonzalo', name: '貢薩洛', occupation: '孩童', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [ingredient('桃子'), ingredient('檸檬')] },
  { id: 'isabel', name: '伊莎貝爾', occupation: '孩童', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('奶香'), effect('甜味')] },
  { id: 'frotga', name: '弗羅特加', occupation: '管家', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('促進消化'), effect('補充水分'), effect('補充精力')] },
  { id: 'lismon', name: '利斯蒙', occupation: '女僕', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('舒緩呼吸')] },
  { id: 'heloise', name: '埃洛伊絲', occupation: '貴婦', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [ingredient('桃子'), effect('煥亮肌膚')] },
  { id: 'oliver', name: '奧利佛', occupation: '牛奶商人', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('輔助瘦身'), effect('紓解壓力')] },
  { id: 'ambrosia', name: '安布羅西婭', occupation: '貴婦', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('防止過食')] },
  { id: 'tiffany', name: '蒂芙尼', occupation: '貴婦', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [ingredient('梨'), effect('奶香'), effect('紓解壓力')] },
  { id: 'petra', name: '佩特拉', occupation: '貴婦', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [ingredient('蘋果'), effect('調節血糖'), effect('辛香')] },
  { id: 'nantelma', name: '南特爾瑪', occupation: '貴婦', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [ingredient('葡萄柚'), effect('煥亮肌膚'), effect('保護心臟')] },
  { id: 'milon', name: '米隆', occupation: '書商', villageId: 'ibex-statue', satisfactionRequired: 0, preferences: [effect('改善視力'), effect('調節血糖')] },
  { id: 'thorsten', name: '托爾斯滕', occupation: '領主', villageId: 'ibex-statue', satisfactionRequired: 350, preferences: null },
  { id: 'savius', name: '薩維烏斯', occupation: '領主', villageId: 'ibex-statue', satisfactionRequired: 250, preferences: null },

]
