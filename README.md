...
22. PR #68 完成 **Candidate-2B｜多層果汁調和搜尋**；PR #69 完成 **Correctness-2｜剩餘果汁終局罐需求診斷**；PR #70 完成 **Performance-1｜配方分頁／篩選與批次規劃 render 降載**。下一個 correctness 小切片是 **Correctness-2B｜允許玩家明確選擇倒掉既有果汁**：預設關閉，只有 opt-in 後才能釋放被既有內容鎖住的實體罐，且交易預覽／正式套用必須明列被倒掉的罐、配方與杯數。完成後再回 Candidate-3。路線最佳化仍等待跨村移動時間、位置資訊、完整顧客服務時段與商店營業時間資料。
