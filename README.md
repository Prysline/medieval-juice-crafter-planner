...
**Inventory-Intermediate I1～I2 已完成。**目前下一步是 **I3｜UI + transaction**：批次規劃加入可搜尋的中間果汁庫存編輯，並讓 whole-plan / partial-delivery commit 正式扣除已使用 intermediate stock、保留未使用 stock；v1 仍不把本次新製作但未使用的 intermediate output 自動持久化。完整 Delivery-Order Correctness 仍排在 Inventory-Intermediate 之後；其後依序 Candidate-3 → Candidate-4 → Phase 6 → Candidate-5。PR #88 維持 Draft prototype-only，不直接 merge。路線最佳化仍等待跨村移動時間、位置資訊、完整顧客服務時段與商店營業時間資料。
