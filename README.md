...
- PR #140～#142 補齊多趟販售的期末剩餘與同配方預補裝 correctness：規劃摘要的「剩餘杯」以實際 physical sales plan 的 `totalLeftoverServings` 為 authority；某罐在較早趟次完成最後販售後留下成品，即使後續趟次不再攜帶該罐，期末內容仍保留並顯示實體罐明細。排程會在不增加趟數、倒掉杯／果汁、必要洗杯用水或換罐次數，且不改變同一 physical jar load sequence 時，嘗試把 terminal leftover 的形成延後。另若同一罐已有同配方成品，且下一個同配方新製作 load 可在容量內先補入，規劃器會建立預補裝候選；**只有該候選確實減少販售趟數且不惡化上述物流指標時才採用**，否則保留原 baseline，避免低杯數或既有 terminal-leftover 時序被意外改壞。
