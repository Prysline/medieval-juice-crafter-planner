...
目前下一步是 **Workflow-2｜製作步驟 checkbox**：建立 stable production-plan fingerprint、任意順序勾選／取消、同 plan 持久恢復與「全部取消／重新開始」；checkbox 只追蹤玩家進度，不改 inventory / optimizer / transaction。完成後才做 **Workflow-3｜果汁分配 delivery checkbox + partial delivery transaction**，再回 Debug-D Production P4。其後依序 Inventory-Intermediate → Candidate-3 → Candidate-4 → Phase 6 → Candidate-5。路線最佳化仍等待跨村移動時間、位置資訊、完整顧客服務時段與商店營業時間資料。
