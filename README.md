...
src/
  ...
  domain/
    ...
    recipeSearch.ts    # 共用搜尋：first-feasible 保留 early stop；bounded-exhaustive 在既有 budget 內收集比較候選，並沿用重複調味後備邊界
...
22. PR #68 完成 **Candidate-2B｜多層果汁調和搜尋**；PR #69 完成 **Correctness-2｜剩餘果汁終局罐需求診斷**；PR #70 完成 **Performance-1｜配方分頁／篩選與批次規劃 render 降載**；PR #72 完成 **Correctness-2B｜明確允許倒掉既有果汁**；PR #74 完成 **Correctness-3｜候選搜尋 consumer 語意拆分**。Correctness-3 保留 `first-feasible` 的 early stop，並讓顧客完整列表／最低成本推薦／optimizer 改用既有 budget 內的 `bounded-exhaustive`；Candidate-2A / 2B 的 generator、搜尋上限、排序、Blender gate、ambiguity 與 unknown-price 邊界都未改動。下一步是 **Objective-1｜批次規劃「最高原料成本」**，之後依序處理果汁罐即時搜尋、配方／顧客研究 UX，再回 Candidate-3 / Candidate-4。路線最佳化仍等待跨村移動時間、位置資訊、完整顧客服務時段與商店營業時間資料。
