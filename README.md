...
- GitHub Pages / HiGHS production runtime 不能只以 build／deploy success 判定。production build 會另外產出 noindex 的 `highs-runtime-smoke.html`；CI 以正式 `/medieval-juice-crafter-planner/` base path 啟動 Pages-like artifact，再由真實 `optimizerWorker.ts` 進入 `Model.solve()`。smoke 專用診斷會在 Worker 內攔截該次實際 `.wasm` response，要求 request URL 位於正式 Pages 子路徑、HTTP 200、`Content-Type: application/wasm`、前四 bytes 為 `00 61 73 6d`，並且 HiGHS 真正完成求解；GitHub Pages workflow 另在 deploy 完成後對部署 URL 執行同一 browser runtime smoke。一般批次規劃不啟用這組診斷，也不改 optimizer objective／certificate／candidate policy。
- Vite dev 也有獨立真實 runtime smoke：`npm run smoke:highs:dev` 會以 repo 的實際 base 啟動 dev server、建立真正 optimizer Worker 並驗證 HiGHS WASM response URL / status / content-type / magic bytes / solve。`npm run dev` 使用 `vite --force`，每次啟動都忽略舊 dependency-prebundle cache，避免先前已把 `highs-ts` 搬進 `.vite/deps` 的本機 cache 繼續解析錯誤 WASM 路徑。乾淨 Linux 與 Windows runner 均已驗證 Vite 7.1.5 可從 `/medieval-juice-crafter-planner/node_modules/@bubblyworld/highs-ts/build/highs.wasm` 取得 `200 application/wasm` 與 `00 61 73 6d`。
...
44. Stage 7 實測資料同步：階段七門檻確認為東港村顧客 29 + 靜謐噴泉顧客 15，達標後寄信給爺爺並在收到回信後解鎖「高級柑橘榨汁機」。木匠售價 1200；信件只明確說明新設備可節省柑橘榨汁時間，實際每批容量、處理時間與效率倍率尚未確認，因此不修改現有柑橘榨汁 production rule。階段七後半另解鎖新 Region「羱羊雕像」，語意與階段四後半的靜謐噴泉相同；羱羊雕像後一般原料包含桃子、黃瓜、番茄、丁香。桃子／黃瓜／番茄已直接確認可由榨汁機處理；丁香已確認為調味器使用的調味材料（seasoning）。牛奶另有液料調和器購買門檻，因此尚未直接加入 runtime。
...
51. PR #173 新增 **批次規劃求解階段與耗時可視化**：執行時顯示「載入規劃模組 → 背景最佳化求解 → 整理製作／販售排程」與即時已耗時；完成後保留本次總耗時、配方證據模式與顧客數。階段耗時不是線性，暫不顯示容易誤導的百分比；本切片不修改 HiGHS Worker protocol、optimizer objective、候選或 correctness。
